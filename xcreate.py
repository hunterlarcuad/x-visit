...
from proxy_api import get_proxy_list
...
def send_msg(x_create, lst_success):
    if len(DEF_DING_TOKEN) > 0 and len(lst_success) > 0:
        ...
        d_cont = {
            'title': 'X Create Finished! [xcreate]',
            'text': (
                'X Account Create [xcreate]\n'
...
python xcreate.py --create --vpn_list --email=@xxone.xyz --headless
python xcreate.py --create --vpn_list --email=@xxone.xyz --sleep_sec_min=600 --sleep_sec_max=3600 --headless
"""
